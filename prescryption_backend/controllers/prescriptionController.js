...
exports.searchMedications = async (req, res) => {
  try {
    ...
    const simplified = results.map(({ brandName, genericName, activeComponentsList, details }) => ({
      brandName,
      genericName,
    ...
    return res.status(500).json({ message: "Error retrieving medication list." });
  }
};
...
exports.issuePrescription = async (req, res) => {
  const {
    patientName, patientSurname, patientNid,
    affiliateNum, insuranceName, insurancePlan,
    med1, quantity1, med2, quantity2,
  ...
  try {
    ...
    // Validación med1
    if (
      !med1.brandName || !med1.details ||
      (!med1.details.saleType?.includes("Venta Libre") && !med1.details.presentation)
    ) {
      return res.status(400).send("Med1 incompleto: presentación requerida si no es de venta libre.");
    }

    // Validación opcional med2
    if (med2) {
      if (
        !med2.brandName || !med2.details ||
        (!med2.details.saleType?.includes("Venta Libre") && !med2.details.presentation)
      ) {
        return res.status(400).send("Med2 incompleto: presentación requerida si no es de venta libre.");
      }
    }
    ...
    console.error("❌ Error issuing prescription:", err);
    return res.status(500).send("Error issuing prescription. Details: " + err.message);
  }
};
