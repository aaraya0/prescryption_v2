    ...
    return (req, res, next) => {
        ...
        try {
            ...
            if (requiredUserType && decoded.userType !== requiredUserType) {
                console.log(`❌ Acceso denegado. Se requiere ${requiredUserType}, pero el usuario es ${decoded.userType}`);
                return res.status(403).json({ message: `❌ Access denied for ${decoded.userType}` });
            }
